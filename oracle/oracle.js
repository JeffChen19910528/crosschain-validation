...
const { CHAINS, wsUrl } = require("../lib/chains");
...
async function init() {
  ...
  const loadA = loadContract(webA, CHAINS.A.buildPath);
  const loadB = loadContract(webB, CHAINS.B.buildPath);
  ...
  // 使用邏輯 ID（與 lib/chains.js 一致），不讀 eth_chainId（Hardhat 兩條鏈都回傳 31337）
